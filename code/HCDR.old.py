...
def bureau_and_balance(num_rows=None, nan_as_category=True):
    ...
    bb_agg = bb.groupby('SK_ID_BUREAU', as_index=True).agg(bb_aggregations)
    ...
    bureau_agg = bureau.groupby('SK_ID_CURR', as_index=True).agg({**num_aggregations, **cat_aggregations})
    bureau_agg.columns = pd.Index(['BURO_' + e[0] + "_" + e[1].upper() for e in bureau_agg.columns.tolist()])
    # Bureau: Active credits - using only numerical aggregations
    active = bureau[bureau['CREDIT_ACTIVE_Active'] == 1]
    active_agg = active.groupby('SK_ID_CURR', as_index=True).agg(num_aggregations)
    active_agg.columns = pd.Index(['ACT_' + e[0] + "_" + e[1].upper() for e in active_agg.columns.tolist()])
    bureau_agg = bureau_agg.join(active_agg, how='inner')
    ...
    closed_agg = closed.groupby('SK_ID_CURR', as_index=True).agg(num_aggregations)
...
def main(debug=False):
    ...
    # add installments payments data
